...
class PostGraph
{
    ...
    var $xTicks=30;  // Number of Ticks on y axe
    ...
    var $fullYaxe=true;    
    var $fullXaxe=true;    
    var $dispMode="C"; // "C"=curve; "B"=Bars
    var $curveThick=2; // epaisseur de la ligne de courbe
    var $BarWidth=0.75;
    var $PrintDataValues=false; // print data values at the top of the bars
    ...
    */
    function drawXAxe()
    {
        ...
        $step = round(( ($this->posXEnd - $this->posXStart) / (min($this->countData,$this->xTicks))), 2);

        for($i=0; $i<=min($this->countData,$this->xTicks); $i++) 
        {
            ImageLine($this->img, $startPos, ($this->fullXaxe ? $this->posYStart : $this->posYEnd -5) , $startPos,  $this->posYEnd+5, $this->colorLines);
	 ...
	 for($i=0; $i<=min($this->countData,$this->xTicks); $i++) 
        {
	    if ($i==min($this->countData,$this->xTicks))
		 { $key=$this->maxX;}
	    else $key=floor($i*$this->maxX/min($this->countData,$this->xTicks));
	
          
            if($this->textXOrientation == 'horizontal') {
                if ($key==$this->maxX) $startPos-=$step/4;
    ...
    */
	...
	$first=true;
	    ...
	    if ($this->dispMode=="C") {
		if ($first) {$valuep=$value; $keyp=$key;}
		ImageLineThick($this->img,$this->posXStart + (($this->posXEnd-$this->posXStart)*$keyp)/$this->maxX , $this->posYEnd - (($this->posYEnd-$this->posYStart)*$valuep)/$this->maxData,$this->posXStart + (($this->posXEnd-$this->posXStart)*$key)/$this->maxX ,$this->posYEnd - (($this->posYEnd-$this->posYStart)*$value)/$this->maxData , $this->colorBars, $this->curveThick);
		$first=false;
		$keyp=$key;
		$valuep=$value;
	    	...
	    	$barWidth = ($this->BarWidth*($this->posXEnd-$this->posXStart)/$this->countData)/2;
           ...
           // sert uniquement pour la position des nombres 
	   $startX = $startPos + $step - (strlen($value)*3);
            
	    if ($this->PrintDataValues) {
