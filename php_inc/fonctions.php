...
// liste d�oulante: nbre val suffisantes et pas de forcage 
  ...
  $retVal.= "</SELECT>";
  $retVal.= (($Mult!="no" && $DispMsg) ? "<br/><small>Appuyez sur Ctrl pour s�ectionner plusieurs valeurs</small>" : "");} // fin liste d�oulante
...
global $debug, $DBName;
  ...
  for ($i=0;$i<db_num_fields($resreq);$i++) {
      ...
      $CIL[$NmChamp]->InitPO();
	$strdbgIPOR.=$NmChamp.", ";
	$CIL[$NmChamp]->ValChp=$tbValChp[$i];
    } // fin boucle sur les champs du r�ultat
  if ($debug) echo("Champs trait� par la fct InitPOReq :".$strdbgIPOR."<br/>\n");
  return($CIL);
}

// fonction qui renvoie un tableau contennant libelle:|valeur, la valeur etant calculée via PYOBj et ramenée qui si est elle est significative
function RTbVChPO($req,$db) {
	$TbPYAObj=InitPOReq($req,$db);
	foreach ($TbPYAObj as $PYAObj) {
		$PYAObj->TypEdit="C";
		$PYAObj->DirEcho=false;
		if ($PYAObj->ValChp != "" && $PYAObj->ValChp != "NULL") 
			$valret[]=$PYAObj->Libelle.":|".$PYAObj->EchoEdit(false);
	}
return $valret;
}
